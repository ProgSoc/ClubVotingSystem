	...
	const fns = {
		...
		async getQuestionVote(
			...
			switch (question.details.type) {
				case "SingleVote": {
					const allSingleCandidateVotes =
						question.originalDbQuestionDataObject.candidates.flatMap(
							(candidate) => candidate.singleCandidateVotes,
						);

					const votes = allSingleCandidateVotes.filter(
						(vote) => vote.voter.id === votingKey,
					);
				...
				case "PreferentialVote": {
					const allPreferentialCandidateVotes =
						question.originalDbQuestionDataObject.candidates.flatMap(
							(candidate) => candidate.preferentialCandidateVotes,
						);

					const votes = allPreferentialCandidateVotes.filter(
						(vote) => vote.voter.id === votingKey,
					);
