...
export async function* subscribeToBoardNotifications(roomId: string) {
	yield withRoomVoterFunctions(roomId, async (fns) =>
		fns.getCurrentBoardState(),
	);

	const newNotifications = pubSub.subscribe("roomBoardEvents", roomId);
...
export async function* subscribeToVoterNotifications(
	...
	yield withRoomVoterFunctions(roomId, async (fns) =>
		fns.getCurrentVoterState(votingKey),
	);
