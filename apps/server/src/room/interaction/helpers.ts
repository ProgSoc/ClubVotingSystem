	...
	const fns = {
		...
		async notifyEveryoneOfBoardChange() {
			...
			voters.admitted.forEach((voter) => {
				pubSub.publish("roomVoter", `${roomId}-${voter.votingKey}`, {
					data: voterState,
				});
			});
		},
		...
		async getShowingQuestionState(
			question: RoomQuestion,
		): Promise<ShowingQuestionState> {
		...
		async getShowingResultsState(
			question: RoomQuestion,
		): Promise<ShowingResultsState> {
