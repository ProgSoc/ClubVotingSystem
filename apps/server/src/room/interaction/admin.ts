...
function makeRoomAdminFunctions(roomId: string) {
	const { questionFns, voterFns, roomFns, ...helpers } =
		makeQuestionHelpers(roomId);
	...
	const fns = {
		...
		async admitUser(userId: string) {
			...
			pubSub.publish("userWaitingList", userId, {
				data: RoomUserState.admitted({ id: userId, votingKey }),
			});
		...
		async declineUser(userId: string) {
			...
			pubSub.publish("userWaitingList", userId, {
				data: RoomUserState.declined({ id: userId }),
			});
		...
		async kickVoter(userId: string) {
			...
			if (user?.votingKey) {
				...
				pubSub.publish("roomVoter", `${roomId}-${user.votingKey}`, {
					data: VoterState.kicked({}),
				});
				await helpers.notifyAdminsOfUsersChanged();
				await helpers.notifyEveryoneOfBoardChange();
			}
...
export async function* subscribeToUserListNotifications(
	...
	yield withRoomAdminFunctions(roomId, adminKey, async (fns) =>
		fns.currentRoomUsersList(),
	);
