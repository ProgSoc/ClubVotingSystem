...
type VariantName<
	...
	Name extends string,
> = `$${Prefix}_${Name}_state`;
function makeVariantName<P extends string, K extends string>(
	prefix: P,
	key: K,
): VariantName<P, K> {
...
type CreateVariantFn<
	...
	Name extends string,
	Obj extends object,
> = (obj: Obj) => Variant<VariantName<Prefix, Name>, Obj>;
...
> {
	...
	match: <Ret>(
		...
		matcher:
			| MakeMatcherForEnum<StateEnum<P, O>, Ret>
			| MakePartialMatcherForEnum<StateEnum<P, O>, Ret>,
	) => Ret;
...
type StateObject<
	P extends string,
	O extends Record<string, Empty<any>>,
> = StateObjectCreateFns<P, O> & StateObjectHelperFields<P, O>;

/** Extract the inner record from StateEnum */
type StateEnumInnerObj<
	SE extends StateEnum<string, Record<string, Empty<any>>>,
> = SE extends StateEnum<string, infer O> ? O : never;

/** Extract the prefix from StateEnum */
type StateEnumInnerPrefix<
	SE extends StateEnum<string, Record<string, Empty<any>>>,
> = SE extends StateEnum<infer P, any> ? P : never;

/** Convert the enum type (and the data of the empty's inside) into a discriminated union */
type RecordToStatesUnion<
	P extends string,
...
> = (
	obj: RecordToStatesUnion<Prefix, Reords>,
) => obj is Variant<VariantName<Prefix, Name>, Obj>;

type MakeIsFnsForStates<
	P extends string,
	O extends Record<string, Empty<any>>,
> = {
	[K in keyof O]: K extends string
...
>(prefix: P, items: O) {
	...
	for (const key in items) {
		...
		objectCreate[key] = ((obj: any) => ({
			...obj,
			[variantField]: result[key],
		})) as any;
...
type MakePartialMatcherForEnum<Enum extends StateEnum<any, any>, Ret> = Partial<
	MakeMatcherForEnum<Enum, Ret>
> & {
	_: () => Ret;
};
