...
export function path<Params extends string>(strings: TemplateStringsArray, ...expr: Params[]): UrlData<Params> {
...
export function withParams<T extends readonly string[]>(
  ...
  const NewComponent = () => {
    ...
    const filtered = Object.entries(params).filter(([key]) => paramsList.includes(key));
    const filteredParams = Object.fromEntries(filtered) as { [K in T[number]]: string };
...
function asDomRoute<Params extends string>(data: RouteData<Params>): RouteObject {
  return {
